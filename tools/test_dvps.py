    ...
    for i, data in enumerate(data_loader):
        ...
        if with_semantic_input:
            semantic_input = mmcv.imread(
                os.path.join(
                    "data/kitti-dvps/semantic/",
                    "{:06d}_{:06d}_semantic.png".format(seq_id, img_id),
                ),
                flag="unchanged",
            )
        ...
        with torch.no_grad():
            segm_results = model(
                return_loss=False, rescale=True, semantic_input=semantic_input, **data
            )
        ...
        dep_path = os.path.join(
            out_dir, "depth", seq_folder, "{:06d}_{:06d}.png".format(seq_id, img_id)
        )
        vis_path = os.path.join(
            out_dir, "vis", seq_folder, "{:06d}_{:06d}.png".format(seq_id, img_id)
        )
        depth_final_rescale = (
            mmcv.imresize(depth_final, (300, 100), interpolation="bilinear")
        ...
        if depth_final_rescale is not None:
            mmcv.imwrite(
                (
                    (depth_final_rescale if rescale_depth else depth_final) * 256.0
                ).astype(np.uint16),
                dep_path,
            )
...
def parse_args():
    ...
    parser.add_argument("--semantic", action="store_true", help="semantic input")
    parser.add_argument("--rescale-depth", action="store_true", help="")
    parser.add_argument("--with-seq", action="store_true", help="")
    parser.add_argument(
        "--launcher",
...
def main():
    ...
    outputs, pred_pans_2ch = single_gpu_test(
        model,
        data_loader,
        args.show,
        args.show_dir,
